...
%% uiextras.gTable.Table - Class definition for Table
%   The Table object places a groupable Java table control within a figure or
%   container.
%% Syntax:
%   t = uiextras.gTable.Table
%   t = uiextras.gTable.Table('Property','Value',...)
%
%% uiextras.gTable.Table Properties:
%
...
%
%% uiextras.gTable.Table Example:
%
%   % Create the figure and display the table
%   f = figure;
%   t = uiextras.gTable.Table('Parent',f);
...
% the thread-safe EDT.
    ...
    properties (Dependent = true) %groupable proerties
        GroupColumns; %which columns should be groupable
        isGroupEnabled;
    end
    
    properties(Dependent = true) %drag
        DraggableRows;
        DragCallback;
    end
    ...
    methods
        function obj = Table(varargin)
            %  uiextras.gTable.Table - Constructor for Table
            % -------------------------------------------------------------------------
            % Abstract: Constructs a new Table object.
            %
            % Syntax:
            %           obj = uiextras.gTable.Table('p1',v1,...)
            %
            % Inputs:
            %           Property-value pairs
            %
            % Outputs:
            %           obj - uiextras.gTable.Table object
            %
            % Examples:
            %           hFig = figure;
            %           obj =  uiextras.gTable.Table('Parent',hFig)
            %
            ...
            p.addParameter('GroupColumns',[]);
            p.addParameter('DragCallback',[]);
            p.addParameter('DraggableRows',false);
            p.parse(varargin{:});
            
            % Add customizations to Java path
            uiextras.gTable.loadJavaCustomizations();
            ...
            obj.DataChangeListener = addlistener(obj,'DataChanged',@(~,~)obj.dataChangeCB);
    ...
    methods (Access = private)
        function create(obj, ParamsToSet)
            ...
            % Convert table model to groupTableModel
            GTableModel = javaObjectEDT('com.jidesoft.grid.DefaultGroupTableModel',jTableModel);
            
%             jTableModel = javaObjectEDT('javax.swing.table.DefaultTableModel',GTableModel.getActualModel());
%             jTableModel = handle(jTableModel, 'callbackproperties');
%             jTableModel.TableChangedCallback = @obj.onTableModelChanged;
            
            % Create table
            %jTable = javaObjectEDT('javax.swing.JTable', jTableModel);
            jTable = javaObjectEDT('com.jidesoft.grid.GroupTable', GTableModel);
            ...
            dragHandler = javaObjectEDT('mtDragTransferHandler');
            jTable.setTransferHandler(dragHandler);
            dragHandler = handle(dragHandler,'CallbackProperties');
            ...
            obj.GTableModel = GTableModel;
            obj.dragHandler = dragHandler;
    ...
    methods
        ...
        function redraw(obj)
            ...
            obj.GTableModel.groupAndRefresh();
        ...
        function refreshRenderers(obj)
            ...
            % renderers.
            rObj = uiextras.gTable.TableRenderers.getRenderers();
    ...
    methods 
        function dataChangeCB(obj,~,~)
            %refresh the table groups
            obj.GTableModel.groupAndRefresh();
        end %function
    end %methods
    ...
    methods
        ...
        function set.ColumnFormat(obj, value)
            % Validate the input
            [StatusOk, Message] = uiextras.gTable.TableRenderers.validateColumnFormat(value);
            if ~StatusOk
                error(Message);
            end
...
%             %RAJ - removed below because setting the value in the model
%             %overwrites the ColumnEditable values
            obj.JTableModel.setColumnIdentifiers(value)
            obj.redraw;
        ...
        %GroupColumns
        function value = get.GroupColumns(obj)
            value = zeros(1,obj.GTableModel.getGroupColmunCount);
            for n=1:obj.GTableModel.getGroupColmunCount
                value(n) = obj.GTableModel.getGroupColumnAt(n)+1;
            end
        end
        function set.GroupColumns(obj,value)

            obj.GTableModel.clearGroupColumns();
            for n=1:numel(value)
                value(n)=value(n)-1;
                if value(n) >= 0 && value(n)<obj.GTableModel.getColumnCount()
                    obj.GTableModel.addGroupColumn(value(n));
                end
            end
            obj.GTableModel.groupAndRefresh();
        ...
        %DraggableRows
        function value = get.DraggableRows(obj)
            value = obj.JTable.getDragEnabled;
        end
        function set.DraggableRows(obj,value)
            value = logical(value);
            obj.JTable.setDragEnabled(value);
        end
        
        %DragCallback
        function set.DragCallback(obj,value)
            if ~(isa(value,'function_handle') || isempty(value))
                error('Value must be a function handle');
            end
            
            if isempty(value)
                obj.DragCB_fcn = [];
                obj.dragHandler.DropTransferCompleteCallback = [];
            else
                obj.DragCB_fcn = value;
                obj.dragHandler.DropTransferCompleteCallback = value;
            end
        end
        function value = get.DragCallback(obj)
            value = obj.DragCB_fcn;
        end
    ...
    methods
        function act_row = getActualRow(obj,vis_row)
            if ~isnumeric(vis_row)||~isscalar(vis_row)
                error('vis_row must be numeric scalar');
            end
            act_row = obj.GTableModel.getActualRowAt(vis_row);
        end
        function setSelectedDataRows(obj,rows)
            %set the selected rows based on their index in the data array
            %collapsed rows will automatically be expanded
            
            row_array = javaArray('com.jidesoft.grid.Row',numel(rows));
            for n=1:numel(rows)
                row_array(n) = obj.GTableModel.getReferenceRow(rows(n)-1);
            end
            obj.JTable.setSelectedRows(row_array);
        end
    end
end % classdef
